...
def metaguide_file(filepath):
    ...
    try:
        metaguiding.metaguide_epub_file(
            filepath, filepath, remove_metaguiding=False
        )
    except Exception as e:  # pylint: disable=broad-except
        common.log.error(
            "Error processing file: %s" % (filepath)
        )
        common.log.error(e)
        raise e
...
class KoboTouchMetaguider(KOBOTOUCH):
    ...
    description = 'Kobo Touch driver with metaguiding support for epub and kepub files.' \
    ...
    def upload_books(self, files, names, on_card, end_session=True, metadata=None):
        ...
        return super().upload_books(files=processed_files,
                                    names= names, 
                                    on_card = on_card, 
